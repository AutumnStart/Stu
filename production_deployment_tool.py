    ...
    def generate_report(self, predictions_df, features_df, output_path="data"):
        ...
        try:
            ...
            with pd.ExcelWriter(data_file, engine='xlsxwriter') as writer:
                ...
                # 找到素材ID列的索引
                id_col_idx = full_report.columns.get_loc('素材ID') # type: ignore
...
def filter_potential_materials(input_file, output_prefix="有潜力素材数据", output_path="data"):
    ...
    try:
        ...
        # 统计各类建议操作数量
        if '建议操作' in filtered_df.columns:
        ...
        try:
            ...
            with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
                ...
                # 找到素材ID列的索引
                id_col_idx = filtered_df.columns.get_loc('素材ID') # type: ignore
...
def get_dataframe_hash(df: pd.DataFrame):
    ...
    try:
        ...
        if '素材ID' in df_copy.columns:
            df_copy = df_copy.sort_values(by=['素材ID']).reset_index(drop=True) # type: ignore
...
def match_with_source_data(potential_materials_file, source_data_file, output_prefix="筛选完成数据", output_path="data"):
    ...
    try:
        ...
        if '日期' in matched_df.columns:
            ...
            if len(total_rows) > 0:
                print(f"✅ 匹配结果 - 找到 {len(total_rows)} 条匹配记录(日期='全部')，涉及 {pd.Series(total_rows['素材ID']).nunique()} 个素材")
                matched_df = total_rows
            else:
                print("⚠️ 未找到日期为'全部'的记录，将使用所有匹配记录")
        
        print(f"📊 最终匹配结果 - {len(matched_df)} 条记录，涉及 {pd.Series(matched_df['素材ID']).nunique()} 个素材")
        
        # 新增：添加“是否裂变”和“是否迭代”列
        cost_column_found = None
        for col_name in ['整体消耗', '消耗', '总消耗', '花费', 'cost']:
            if col_name in matched_df.columns: # type: ignore
                cost_column_found = col_name
                break
        
        if cost_column_found:
            print(f"✅ 使用 '{cost_column_found}' 列添加裂变信息")
            # 清理并转换消耗列为数值类型
            if matched_df[cost_column_found].dtype == 'object':
                matched_df[cost_column_found] = matched_df[cost_column_found].astype(str).str.replace(',', '').str.replace(' ', '').str.replace('¥', '').str.replace('$', '').str.replace('￥', '') # type: ignore
            
            matched_df[cost_column_found] = pd.to_numeric(matched_df[cost_column_found], errors='coerce').fillna(0) # type: ignore

            # 添加“是否裂变”列
            matched_df['是否裂变'] = np.where(matched_df[cost_column_found] > 10000, '是', '否')
            
            print("✅ 已添加 '是否裂变' 列")
        else:
            print("⚠️ 未找到'整体消耗'相关列，无法添加裂变列")
            matched_df['是否裂变'] = '否'
